...
class FeedFile extends Column
{
    ...
    public function getFileLink($filePath)
    {
        ...
        if (isset($filePath) and !empty($filePath)) {
            ...
            $file = $this->dl->getPath('media') . "/Betterthat/feed/" . $fileName;
            if ($this->file->fileExists($file)) {
                $url = $this->urlBuilder->getBaseUrl() . "pub/media/Betterthat/feed/" . $fileName;
            } else {
                if ($this->file->fileExists($filePath)) {
                    $cpDir = $this->dl->getPath('media') . "/Betterthat/feed/";
