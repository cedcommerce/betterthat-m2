...
*/
    ...
    public function getResponseFileLink($filePath)
    {
        ...
        if (isset($filePath) and !empty($filePath)) {
            ...
            $file = $this->dl->getPath('media')."/mpcatch/response/" . $fileName;
            if ($this->file->fileExists($file)) {
                $url = $this->urlBuilder->getBaseUrl() . "pub/media/mpcatch/response/" . $fileName;
            } else {
                if ($this->file->fileExists($filePath)) {
                    $cpDir = $this->dl->getPath('media')."/mpcatch/response/";
