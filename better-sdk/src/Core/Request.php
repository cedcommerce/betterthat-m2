   ...
   /**
    ...
    public function responseParse($response = false, $type = self::FEED_CODE_ITEM_UPDATE, $filePath = '')
    {
        ...
        $feedResponse = array(
            ...
            'feed_type' => $type,
            'feed_status' => 'Betterthat api failed. Kindly check the api logs.',
            'feed_errors' => "{\"Api Failure\":\"Betterthat api failed. Kindly check the api logs.\"}",
            'feed_file' => $filePath
        );
        ...
        if ($type && $response) {
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->debug(
                        "BetterthatSdk\\Core\\Request\\responseParse(): \n Response : \n" . var_export($response, true) .
                        "\n Exception : \n" . var_export($e->getMessage(), true)
                    );
                }
            }
    ...
    public function getFeeds($feedId = null, $subUrl = self::GET_FEEDS_SUB_URL)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    sprintf(
                        "BetterthatSdk\\Core\\Request\\getFeeds(): \n Response : \n %s \n Exception : %s \n",
                        var_export($response, true),
                        var_export($e->getMessage(), true)
                    )
                );
     ...
     * @param string|[] $params
     * @return string
     * @errors 1. <api-response xmlns="http://seller.marketplace.Betterthat.com/shared/v1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemalocation="http://seller.marketplace.Betterthat.com/shared/v1 https://seller.marketplace.Betterthat.com/SellerPortal/s/schema/shared/api-response-v1.xsd">
    ...
    public function validateXml($filePath, $xsdPath, $type = 'lmp-item')
    {
        ...
        if ($result === true) {
            if (($feed->schemaValidate($xsdPath))) {
                ...
                if ($this->debugMode) {
                    $this->logger->debug(
                        "BetterthatSdk\\Core\\Request\\validateXml(): \n" . "\n Errors : \n" . var_export($errorList, true)
                    );
                }
