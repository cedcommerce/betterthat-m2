...
class Order extends \BetterthatSdk\Core\Request
{
    ...
    {
        $response = $response = $this->getRequest($subUrl . '?max=50&order_ids=' . $purchaseOrderIds);
        try {
     ...
     * @return array
     */
    public function refundOnBetterthat($refundData = array(), $url = self::PUT_ORDER_REFUND)
    {
        $xml = new \BetterthatSdk\Core\Generator();
