...
class Product extends \BetterthatSdk\Core\Request
{
    ...
    public function getAttributes($params, $forceFetch = false)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "BetterthatSdk\\Product\\getAttributes() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
        return $attributes;
	...
	/**
     * Create/Update Offer on Betterthat
     * @param $data
     * @return bool|array
     */
    ...
    /**
     * Deactivate Product on Betterthat
     * @param $data
     * @return bool | array
     */
        ...
        if (isset($data[0]['item-to-inactivate'])) {
            $product = [
                'catalog-feed' =>
                    [
                        '_attribute' => [
                            'xmlns' => 'http://seller.marketplace.Betterthat.com/catalog/v22',
                            'xmlns:xsi' => 'http://www.w3.org/2001/XMLSchema-instance',
                            'xsi:schemaLocation' =>
                                'http://seller.marketplace.Betterthat.com/catalog/v22 ../../../../../rest/catalog/import/v22/lmp-item.xsd',
                        ],
    ...
    public function updatePrice($data)
    {
        if (isset($data[0]['item'])) {
            $pricing = [
                'pricing-feed' => [
                    '_attribute' => [
                        'xmlns' => 'http://seller.marketplace.Betterthat.com/pricing/v5',
                        'xmlns:xsi' => 'http://www.w3.org/2001/XMLSchema-instance',
                        'xsi:schemaLocation' =>
    ...
    // Get inventory for all products uploaded on Betterthat for status change
    public function getInventory($skus = [])
    {
        if (!empty($skus) and is_array($skus)) {
            ...
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->addDebug('\BetterthatSdk\Product\getInventory(): Error: ' . $e->getMessage());
                }
                return false;
            }
        }
    ...
    public function getValueLists($attributeCode, $forceFetch = false, $url = self::GET_ATTRIBUTES_VALUE_LIST)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "BetterthatSdk\\Product\\getValueLists() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $valueLists;
        }
    ...
    public function getOffers($chunkSize = 50, $offset = 0, $url = self::GET_OFFERS)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "BetterthatSdk\\Product\\getOffers() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $offers;
        }
