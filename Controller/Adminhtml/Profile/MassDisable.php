...
class MassDisable extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (!is_array($profileIdsToDisable) && !$excluded) {
            ...
            $profileIdsToDisable  = $this->_objectManager->create(\Ced\Betterthat\Model\Profile::class)
        ...
        if (!empty($profileIdsToDisable)) {
            try {
                foreach ($profileIdsToDisable as $profileId) {
                    $profile = $this->_objectManager->create(\Ced\Betterthat\Model\Profile::class)
                        ->load($profileId);
