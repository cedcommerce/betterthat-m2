...
class Order extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /**
     * @var \Betterthat\Betterthat\Helper\Tax $taxHelper
     */
    public $taxHelper;
     ...
     * @param \Betterthat\Betterthat\Model\OrdersFactory $orders
     * @param \Betterthat\Betterthat\Model\FeedsFactory $feedsFactory
     * @param \Betterthat\Betterthat\Model\OrderFailedFactory $orderFailed
     * @param Config $config
     * @param Logger $logger
    ...
    public function __construct(
        ...
        \Betterthat\Betterthat\Model\OrdersFactory $orders,
        \Betterthat\Betterthat\Model\FeedsFactory $feedsFactory,
        \Betterthat\Betterthat\Model\OrderFailedFactory $orderFailed,
        ...
        \Betterthat\Betterthat\Helper\Tax $taxHelper,
        \Magento\Quote\Model\Quote\Address\RateFactory $rateFactory,
        \Magento\Framework\DataObjectFactory $dataFactory,
        \Betterthat\Betterthat\Model\MailFactory $mailFactory,
        \Betterthat\Betterthat\Model\ChangeQuoteControl $quoteControl,
    ...
    public function importOrders($data = null)
    {
        try {
            ...
            if ($this->config->isValid() == "0" || $this->config->isValid() == null) {
                $this->messageManager->addErrorMessage("Invalid API credentials");
                return;
            }
            ...
            if (isset($response['data']) && count($response['data']) > 0) {
                ...
                foreach ($response as $order) {
                    $order = $this->parseOrderData($order);
                    $BetterthatOrderId = $order['_id'];
                    $this->logger->info(
                        'Order Fetch Response For Order Id : ' . $BetterthatOrderId,
                        [
                            'path' => __METHOD__,
    ...
    /**
     * ParseOrderData Function
     *
     * @param array $order
     * @return array
     */
    public function parseOrderData($order = [])
    {
        // Modifying the Product_data Node In Array
        if (!isset($order['Product_data']) && empty($order['Product_data'])) {
            $productArray = [];
            foreach ($order['Order_product'] as $key => $data) {
                $productArray[] = $data['Product_data'];
                unset($order['Order_product'][$key]['Product_data']);
            }
            $order['Product_data'] = $productArray;
            // End
        } else {
    ...
    public function generateQuote(
        ...
        try {
            ...
            if (isset($order['Product_data'][0])) {
                if (isset($order['source']) && !empty($order['source']) && $order['source'] == 'webhook') {
                    $addedItems = $this->addProductsToQuote($order, $quote);
                } else {
                    $addedItems = $this->addProductsToQuote2($order, $quote);
                }
                $priceArr = $addedItems['priceArr'];
                ...
                if ($failedOrder) {
                    ...
                    $shippingData = isset($order['Shipping_data']) ? $order['Shipping_data'] : [];
                    ...
                    try {
                        ...
                        if (isset($order['shipping_type']) && !empty($order['shipping_type']) &&
                            empty($order['formatted_shipping_type'])) {
                            $shipTitle = isset($titles[$order['shipping_type']]) ?
                                $titles[$order['shipping_type']] : $order['shipping_type'];
                        } elseif (isset($order['formatted_shipping_type']) &&
                            !empty($order['formatted_shipping_type'])) {
                        ...
                        if (isset($magentoOrder) && !empty($magentoOrder)) {
                            ...
                            foreach ($magentoOrder->getAllItems() as $item) {
                                $price = isset($priceArr[$item->getProductId()]) ?
                                    $priceArr[$item->getProductId()] : null;
                                $item->setBasePrice($price);
                                $item->setSpecialPrice($price);
                                $item->setDiscountAmount(0);
                                $item->setBaseDiscountAmount(0);
    ...
    /**
     * AddProductsToQuote2 Function
     *
     * @param mixed $order
     * @param mixed $quote
     * @return array
     */
    public function addProductsToQuote2($order, $quote)
    {
        $failedOrder = false;
        $qtyArray = [];
        $reason = [];
        ...
        try {
            foreach ($order['Order_product'] as $prod_data) {
                $qtyArray[$prod_data['variance_id']]['quantity'] = $prod_data['quantity'];
            }
            foreach ($qtyArray as $productId => $quantity) {
                foreach ($order['Product_data'] as $productDatum) {
                    if (!empty($productDatum['variances'])) {
                        foreach ($productDatum['variances'] as $variance) {
                            if ($productId == $variance['_id']) {
                                $sku = $variance['un_sku_code'];
                                $price = $productDatum['rrp'];
                                $qtyArray[$productId]['price'] = $price;
                                $qtyArray[$productId]['sku'] = $sku;
                            }
                        }
                    }
                }
            }
            foreach ($qtyArray as $id => $item) {
                    ...
                    if (isset($product) && !empty($product)) {
                        ...
                        if ($product->getStatus() == '1') {
                            ...
                            if ($stockStatus) {
                                ...
                                $reason[] = "Sku : " . $sku . " | Product Id : " . $id . " Is Out Of Stock";
                                $failedOrder = true;
                                break;
                            }
                        } else {
                            $reason[] = "Sku : " . $sku . " | Product Id : " . $id . " Is Not Enabled On Store";
                            $failedOrder = true;
                            break;
                        }
                    } else {
                        $reason[] = "No Product Exist With This Sku : " . $sku . " | Product Id : " . $id;
                        $failedOrder = true;
                        break;
                    }
                } else {
                    $reason[] = "No Sku Exist For This Product Id : " . $id;
                    $failedOrder = true;
                    break;
                }
            }
    ...
    public function autoOrderAccept($BetterthatOrderId, $acceptanceArray)
    {
        ...
        $response = $BetterthatOrder
            ->acceptrejectOrderLines($BetterthatOrderId, $acceptanceData);
        $this->logger->info(
            'Auto Accept Order Acceptance Data',
            [
                'path' => __METHOD__,
                'AcceptanceData' => json_encode($acceptanceData)
            ]
        );
        try {
            $BetterthatOrder = $this->orders->create()
    ...
    {
        $to_email = $this->scopeConfig
            ->getValue('betterthat_config/betterthat_order/order_notify_email');
        try {
            if ($to_email) {
                /**
                ...
                /**
                 * @var \Betterthat\Betterthat\Model\Mail $mail
                 */
    ...
    public function submitMagentoShipment(array $data = [])
    {
        ...
        $title = isset($data[0]['tracking_info']['shipping_company'])
            ? $data[0]['tracking_info']['shipping_company'] : 'N/A';
        $trackingNumber = isset($data[0]['tracking_info']['tracking_number'])
            ? $data[0]['tracking_info']['tracking_number'] : 'N/A';
    ...
    public function shipOrder(array $data = [])
    {
        ...
        try {
            ...
            $this->logger->info(
                'Ship Order Tracking Update',
                [
                    'path' => __METHOD__,
                    'ShipData' => var_export($data, true),
                    'TrackingData' => var_export($arraytoship, true),
                    'ShipResponseData' => var_export($response, true)
                ]
            );
            if ($arraytoship) {
                $status = $order->putShipOrder($arraytoship);
                $this->logger->info(
                    'Ship Order Status Update',
                    [
                        'path' => __METHOD__,
                        'ShipData' => var_export($arraytoship, true),
                        'ShipResponseData' => var_export($status, true)]
                );
                return $status;
            } else {
    ...
    public function notificationSuccess($count)
    {
        ...
        $model->setData(
            'description',
            "Congratulation! You have received " . $count . " new orders form Betterthat"
        );
        $model->setData('url', "#");
    ...
    public function generateShipment($order, $cancelleditems)
    {
        ...
        if ($shipment) {
            ...
            try {
                ...
                $this->messageManager
                    ->addErrorMessage('Error in saving shipping:' . $e->getMessage());
                $this->logger
                    ->error(
                        'Generate Magento Shipment',
                        [
                            'path' => __METHOD__,
                            'exception' => $e->getMessage(),
                            'trace' => $e->getTraceAsString()
    ...
    public function shipOrders($BetterthatOrders)
    {
        if (count($BetterthatOrders) == 0) {
            ...
            foreach ($BetterthatOrders as $BetterthatOrder) {
                ...
                $this->order = $this->objectManager->create(\Magento\Sales\Api\Data\OrderInterface::class);
                ...
                if ($order->getStatus() == 'complete' || $order->getStatus() == 'Complete') {
                    ...
                    if ($return) {
                        $this->logger->info(
                            ...
                            [
                                'path' => __METHOD__,
                                'Magento Increment ID' => $magentoOrderId,
                                'Response Data' => var_export($return, true)
                            ]
                        );
                    } else {
                        $this->logger->info(
        ...
        try {
            ...
            if (empty($purchaseOrderId)) {
                $this->logger->info(
                    ...
                    [
                        'path' => __METHOD__,
                        'ShipData' => 'Not A Betterthat Order.'
                    ]
                );
                return false;
            }
    ...
    public function checkStockQtyStatus($product, $qty)
    {
        ...
        if ($useMSI) {
            ...
            if ($invSourceData && is_array($invSourceData) && count($invSourceData) > 0) {
                ...
                $quantity = isset($invSourceData[$msiSourceCode])
                    ? $invSourceData[$msiSourceCode] : 0;
            ...
            $stockStatus = ($stock->getQty() > 0)
                ? ($stock->getIsInStock() == '1' ?
    ...
    public function addTransactionToOrder($order, $BetterthatOrderId)
    {
        try {
            ...
            $this->_transactionBuilder = $this->objectManager
                ->create(\Magento\Sales\Model\Order\Payment\Transaction\BuilderInterface::class);
            ...
            $this->logger->error(
                'Order Payment Transaction Error',
                [
                    'path' => __METHOD__,
                    'exception' => $e->getMessage(),
                    'trace' => $e->getTraceAsString()
    ...
    {
        $this->registry
            ->unregister(\Betterthat\Betterthat\Model\Carrier\Betterthat::REGISTRY_INDEX_SHIPPING_TOTAL);

        $this->registry->register(
            \Betterthat\Betterthat\Model\Carrier\Betterthat::REGISTRY_INDEX_SHIPPING_TOTAL,
            $amount
        );
    }
    ...
    {
        $this->registry
            ->unregister(\Betterthat\Betterthat\Model\Carrier\Betterthat::REGISTRY_INDEX_SHIPPING_TAX_PERCENTAGE);

        $this->registry->register(
            \Betterthat\Betterthat\Model\Carrier\Betterthat::REGISTRY_INDEX_SHIPPING_TAX_PERCENTAGE,
            $amount
        );
    }
}
