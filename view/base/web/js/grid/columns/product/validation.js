        ...
        return Column.extend(
            {
                ...
                startView: function (row) {
                    if (this.getProductvalidation(row)) {
                        var previewPopup = $('<div>',{id : 'Betterthatpopup'+this.getProductId(row) });
                        ...
                        $.each(
                            data, function (index, value) {
                                var messages = '';
                                $.each(
                                    value.errors, function (i, v) {
                                        if (typeof v === 'object' && Object.keys(v).length > 0) {
                                            messages += '<ul style="list-style: none;">';
                                            $.each(
                                                v, function (attribute, err) {
                        ...
                        var Betterthatpopup = previewPopup.modal(
                            {
